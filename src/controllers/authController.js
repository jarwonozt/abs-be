...
exports.getProfile = async (req, res) => {
  try {
    ...
    const result = await db.query(
      'SELECT id, name, email, role, phone, address, office_lat, office_lon, office_radius, shift_start, shift_end, photo, is_active, created_at, updated_at FROM users WHERE id = $1',
      [userId]
    );
    ...
    const result = await db.query(
      `UPDATE users 
       SET name = COALESCE($1, name),
           ...
           updated_at = NOW()
       WHERE id = $4
       RETURNING id, name, email, role, phone, address, office_lat, office_lon, office_radius, shift_start, shift_end, photo, is_active, created_at, updated_at`,
